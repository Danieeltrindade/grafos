...
int main(void)
{
    ...
    if (g == NULL || !inserir_aresta_lista(g, 0, 1) ||
        !inserir_aresta_lista(g, 1, 2) || !inserir_aresta_lista(g, 2, 3) ||
        !inserir_aresta_lista(g, 3, 0) || !inserir_aresta_lista(g, 2, 4)) {
        liberar_grafo_lista(g);
        return EXIT_FAILURE;
    }
    ...
    if (articulacoes == NULL || (quantidade_pontes > 0 && pontes == NULL)) {
        free(articulacoes);
        free(pontes);
        liberar_grafo_lista(g);
        return EXIT_FAILURE;
