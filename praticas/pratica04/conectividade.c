                      ...
                      int articulacao[])
{
    ...
    for (atual = g->adj[u]; atual != NULL; atual = atual->prox) {
        ...
        if (descoberta[v] == 0) {
            ...
            /* A raiz precisa de dois filhos; os demais usam low[v] >= descoberta[u]. */
            if ((pai[u] == -1 && filhos > 1) ||
...
int *detectar_articulacoes(GrafoLista *g)
{
    ...
    if (g->n > 0 && (descoberta == NULL || low == NULL || pai == NULL ||
                     articulacao == NULL)) {
        free(descoberta);
        free(low);
        free(pai);
    ...
    for (u = 0; u < g->n; u++) {
        if (descoberta[u] == 0) {
            dfs_articulacoes(g, u, &tempo, descoberta, low, pai, articulacao);
        }
    }
    free(descoberta);
    free(low);
...
static int adicionar_ponte(Aresta **pontes, int *quantidade, int *capacidade,
                           int u, int v)
{
    if (*quantidade == *capacidade) {
        int nova_capacidade = *capacidade == 0 ? 4 : *capacidade * 2;
        Aresta *novo = realloc(*pontes, (size_t)nova_capacidade * sizeof(*novo));
        if (novo == NULL) {
            return 0;
        }
        *pontes = novo;
...
static int dfs_pontes(GrafoLista *g, int u, int *tempo, int descoberta[],
                      int low[], int pai[], Aresta **pontes, int *quantidade,
                      int *capacidade)
{
    No *atual;
    ...
    for (atual = g->adj[u]; atual != NULL; atual = atual->prox) {
        ...
        if (descoberta[v] == 0) {
            pai[v] = u;
            if (!dfs_pontes(g, v, tempo, descoberta, low, pai, pontes,
            ...
            if (low[v] > descoberta[u] &&
                !adicionar_ponte(pontes, quantidade, capacidade, u, v)) {
                return 0;
            }
        } else if (v != pai[u] && descoberta[v] < low[u]) {
            low[u] = descoberta[v];
...
Aresta *detectar_pontes(GrafoLista *g, int *quantidade)
{
    ...
    *quantidade = 0;
    ...
    for (u = 0; u < g->n; u++) {
        if (descoberta[u] == 0 &&
            !dfs_pontes(g, u, &tempo, descoberta, low, pai, &pontes,
                        quantidade, &capacidade)) {
            free(pontes);
            pontes = NULL;
            *quantidade = 0;
